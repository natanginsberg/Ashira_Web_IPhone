...
enum NextFunction { WATCH_RECORDING, DOWNLOAD_RECORDING, ADD_HARMONY }
...
class _MobileSingState extends State<MobileSing> with WidgetsBindingObserver {
  ...
  void play() async {
    headsetConnected = headsetEvent == HeadsetState.CONNECT;
  ...
  setFileSong(path) {
    ...
    audioPlayer.setFilePath(audioDownloadPath);
    setState(() {
      songPicked = true;
      fileDownloaded = true;
    });
                    ...
                    Container(
                      ...
                      child: TextButton(
                          onPressed: () async {
                            ...
                            if (result != null) {
                              setFileSong(result.files.single.path);
                            } else {
                              // User canceled the picker
                            }
                          },
                          child: Text(
                            AppLocalizations.of(context)!.uploadFromPhone,
                            style: TextStyle(
                              fontSize: 20,
                              color: Colors.white,
                              //    letterSpacing: 1.5
                ...
                child: Stack(children: [
                  Column(
                      ...
                      children: [
                        ...
                        if (!addHarmony && headsetConnected)
                          Container(
                            child: TextButton(
                                onPressed: () async {
                                  setState(() {
                                  ...
                                  if (videoFile == null)
                                    await finishVideoRecording();
                                  nextFunction = NextFunction.ADD_HARMONY;
                                  if (!service.isUserSignedIn()) {
                                    signInOptions(false);
                                  } else {
                                    restartSongWithHarmony();
                                  }
                                },
                                child: Text(
                                  AppLocalizations.of(context)!.addHarmony,
                                  style: TextStyle(
                                    fontSize: 20,
                                    color: Colors.white,
                                    //letterSpacing: 1.5
                        ...
                        if (!addHarmony && headsetConnected) divider(),
                        Container(
                          child: TextButton(
                              onPressed: () {
                                restart();
  ...
  void addUserToFirebase() async {
    ...
    if (userEmail != null) {
      ...
      bool userAdded = await MobileUserHandler().sendUserInfoToFirestore(
          userEmail,
  ...
  concatFiles(double initialDelay, Duration currentPosition,
      [bool preparingHarmony = false]) async {
    var dir = await getApplicationDocumentsDirectory();
    if (preparingHarmony)
      concatedFile = '${dir.path}/audio2_.mp4';
    else
    ...
    String harmonyCommand = addHarmony && !preparingHarmony
        ? "[0:v][1:v]hstack=inputs=2[v];[0:a][a1]amerge=inputs=2[a]\" -map \"[v]\" -map \"[a]\" -ac 2"
        : "[0:a][a1]amerge=inputs=2[a]\" -map 0:v -map \"[a]\" -c:v copy -ac 2";
    if (addHarmony || noiseRecorded) {
      if (fileDownloaded)
        _flutterFFmpeg
            .execute("-ss " +
                delay.toString() +
                ...
                " -i " +
                audioDownloadPath +
                " -filter_complex \"[1:a]volume=0.8[a1];" +
                harmonyCommand +
                " -shortest " +
                concatedFile)
            .then((value) {
  ...
  void shrinkFileAndDownload() async {
    ...
    await checkIfVideoIsSung();
    String harmonyCommand = addHarmony
        // && headsetConnected
        ? "[0:v][2:v]hstack=inputs=2[v];[v][wm]overlay=main_w-overlay_w-5:main_h-overlay_h-5[v0];"
        : "[0:v][wm]overlay=main_w-overlay_w-5:main_h-overlay_h-5[v0];";
    if (addHarmony || noiseRecorded) {
      if (fileDownloaded) {
        var dir = await getApplicationDocumentsDirectory();
        ...
        _flutterFFmpeg
            .execute("-ss " +
                ...
                " -i " +
                audioDownloadPath +
                " -filter_complex \"[1]scale=iw/2:-1[wm];" +
                harmonyCommand +
                "[2:a]volume=0.5[a2];[0:a][a2]amerge=inputs=2[a]\" -map \"[v0]\" -map \"[a]\" -ac 2 -shortest -b:v 1M " +
  ...
  shrinkFileAndDownloadAudioOnly() async {
    readyToDownload = true;
    await checkIfVideoIsSung();
    if (addHarmony || noiseRecorded) {
      print("starting");
      if (fileDownloaded) {
        final folderName = "Downloads";
        final path = Directory("storage/emulated/0/$folderName");
        // var dir = await getExternalStorageDirectories();
        var shrinkedFileToDownload = '$path/ashira_' +
            DateTime.now().millisecondsSinceEpoch.toString() +
            '.mp3';
        _flutterFFmpeg.execute("-ss " +
            delay.toString() +
            " -i " +
            videoFile!.path +
            " -i " +
            ...
            " -i " +
            audioDownloadPath +
            " -filter_complex \"[2:a]volume=0.5[a2];[0:a][a2]amerge=inputs=2[a]\" -map \"[a]\" -ac 2 -shortest " +
            shrinkedFileToDownload);
        // .then((value) => downloadConcatenatedFile(shrinkedFileToDownload));
      }
    } else {
  ...
  void continueWithFunctionRequested() {
    ...
    if (nextFunction != null) {
      ...
      else if (nextFunction == NextFunction.ADD_HARMONY)
        restartSongWithHarmony();
    }
  }
  ...
  void continueDownloadingRecording(bool video) {
    if (video)
      shrinkFileAndDownload();
    else
      shrinkFileAndDownloadAudioOnly();
  }
  ...
  void restartSongWithHarmony() async {
    if (concatedFile == "")
      await concatFiles(delay, new Duration(seconds: 0), true);
    else
      restartWithHarmony();
  }

  void restartWithHarmony() {
    Navigator.of(context).pop(STAY_ON_PAGE);
    audioDownloadPath = concatedFile.substring(0);
    concatedFile = "";
    restart();
    setState(() {
      songFinished = false;
  ...
  Future<void> openDownloadOptions() {
          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: const <Widget>[
                Text('Would you like to download video as well?'),
                Text('(This will make the file much bigger))'),
              ],
            ),
          ),
          actions: <Widget>[
            TextButton(
              ...
              onPressed: () {
                continueDownloadingRecording(true);
                Navigator.of(context).pop();
              },
            ),
            TextButton(
              ...
              onPressed: () {
                continueDownloadingRecording(false);
                Navigator.of(context).pop();
              },
            ),
          ],
